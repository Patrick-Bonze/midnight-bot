require('dotenv').config();

const { 
  Client, 
  GatewayIntentBits, 
  EmbedBuilder, 
  PermissionFlagsBits, 
  ActionRowBuilder, 
  ButtonBuilder, 
  ButtonStyle 
} = require('discord.js');
const mongoose = require('mongoose');
const Dor = require('./models/dor.js');
const User = require('./models/user.js');
const Season = require('./models/season.js');
const Guild = require('./models/guild.js');

//  CONSTS 
const RANKING_BANNER = 'https://i.pinimg.com/1200x/a6/d6/e5/a6d6e5881c691fc7a7a7db21eb17aed5.jpg';
const BITES_THE_DUST_GIF = 'https://static.jojowiki.com/images/2/21/latest/20191015213038/KQ_Bites_the_Dust.gif';
const SEU_ID_DONO = '...';
const messageInterval = new Map();
const MIN_INTERVAL = 10000; // 10 segundos
const DUEL_GIF = 'https://i.pinimg.com/originals/7a/0c/bb/7a0cbb99caad0f4c76020ff8ba3cae26.gif';

//  FUNÇÕES 
const lastMessages = new Map();

function isValidMessage(msg, userId) {
  const content = msg.trim().toLowerCase();
  if (!/[a-zA-Z]/.test(content)) return false;

  // mínimo de caracteres
  if (content.length < 5) return false;

  // repetição tipo "aaaaa"
  if (/^(.)\1+$/.test(content)) return false;

  // só números
  if (/^\d+$/.test(content)) return false;

  // só emoji
  if (/^[\p{Extended_Pictographic}]+$/u.test(content)) return false;

  // mensagem repetida
  const lastMsg = lastMessages.get(userId);
  if (lastMsg === content) return false;

  lastMessages.set(userId, content);

  return true;  
}
function canGainPoint(userId) {
  const now = Date.now();
  const last = messageInterval.get(userId) || 0;

  if (now - last < MIN_INTERVAL) return false;

  messageInterval.set(userId, now);
  return true;
}
async function sendSeasonEmbed(channel, seasonNumber) {
  const topUsers = await User.find({ fama: { $gt: 0 } }).sort({ fama: -1 }).limit(3);

  if (!topUsers.length) return channel.send('A temporada encerrou sem vencedores! 😅');

  const embed = new EmbedBuilder()
    .setTitle(`🏆 HALL DA FAMA — TEMPORADA ${seasonNumber}`)
    .setColor(0xF1C40F)
    .setDescription(`O tempo de glória chegou! Estes são os destaques da temporada.`)
    .setTimestamp();

  const medals = ['👑 Mastermind', '🌙 Moonlight', '☀️ Sunflare'];
  const colors = [0xF1C40F, 0x9B59B6, 0xE67E22];

  for (let i = 0; i < topUsers.length; i++) {
    const userData = topUsers[i];
    const guildMember = await channel.guild.members.fetch(userData.userId).catch(() => null);
    if (!guildMember) continue;

    const roleName = `${medals[i]} S${seasonNumber}`;
    let role = channel.guild.roles.cache.find(r => r.name === roleName);
    if (!role) {
      role = await channel.guild.roles.create({
        name: roleName,
        color: colors[i],
        reason: 'Sistema de temporada',
      });
    }

    if (role.position < channel.guild.members.me.roles.highest.position) {
      await guildMember.roles.add(role).catch(() => {});
    }

    embed.addFields({
      name: `\n${medals[i]}`,
      value: `${i + 1}º Lugar — ${guildMember} — ⭐ ${userData.fama}\nCargo: <@&${role.id}>`,
      inline: false,
    });
  }

  await channel.send({ embeds: [embed] });
}

async function checkSeasonAuto(client) {
  const guildsConfig = await Guild.find();

  for (const config of guildsConfig) {
    const hallChannel = await client.channels.fetch(config.hallChannelId).catch(() => null);
    if (!hallChannel) continue;

    let season = await Season.findOne();
    if (!season) {
      season = new Season({
        start: new Date(),
        seasonNumber: 1,
        durationDays: 30,
      });
      await season.save();
      continue;
    }

    const now = new Date();
    const diffDays = Math.ceil((now - season.start) / (1000 * 60 * 60 * 24));

    if (diffDays >= season.durationDays) {
      if (!season.lastEnd) season.lastEnd = new Date(0);
      const diffMinutes = (now - season.lastEnd) / (1000 * 60);
      if (diffMinutes < 5) continue; // evita rodar duas vezes seguidas

      await sendSeasonEmbed(hallChannel, season.seasonNumber);
      await User.updateMany({}, { fama: 0 });

      season.start = new Date();
      season.seasonNumber += 1;
      season.lastEnd = new Date();
      await season.save();
    }
  }
}

//  CLIENT 
const client = new Client({
  intents: [
    GatewayIntentBits.Guilds,
    GatewayIntentBits.GuildMessages,
    GatewayIntentBits.MessageContent,
    GatewayIntentBits.GuildMembers,
  ],
});

//  DATABASE 
mongoose.connect(process.env.MONGO_URI)
  .then(() => console.log('🟢 Database Conectada'))
  .catch(err => console.error('🔴 Erro na Database:', err));

//  EVENTO READY 
client.once('ready', () => {
  console.log(`🔥 Midnight-BOT Online: ${client.user.tag}`);
  checkSeasonAuto(client);
  setInterval(() => checkSeasonAuto(client), 1000 * 60 * 60);
});

//  EVENTO MESSAGE CREATE 
client.on('messageCreate', async (message) => {
   if (message.author.bot || !message.guild) return;
 console.log(`MSG: ${message.author.tag} - ${message.content}`);

 const content = message.content.trim().toLowerCase();
const args = message.content.split(' ');

  //  COMANDOS BÁSICOS 
  if (content === '!ajuda') {
    const helpEmbed = new EmbedBuilder()
      .setTitle('📖 GUIA DE COMANDOS — MIDNIGHT')
      .setColor(0x3498DB)
      .setDescription('Comandos disponíveis:')
      .addFields(
        {
          name: '⭐ Membros',
          value: '!fama - Veja sua pontuação.\n!ranking - Top 10.\n!var @user - Ver fama de outro usuário.'
        },
        {
          name: '🛠️ Staff',
          value: '!apresentacao\n!sethall #canal\n!addfama @user valor\n!remfama @user valor\n!fimseason\n!resetseason\n!bitesthedust'
        }
      );
    return message.reply({ embeds: [helpEmbed] });
  }

  if (content.startsWith('!var')) {
    const target = message.mentions.users.first();
    if (!target) return message.reply('❌ Use: !var @usuario');
    const user = await User.findOne({ userId: target.id });
    return message.reply(`⭐ ${target} possui **${user ? user.fama : 0}** de fama`);
  }
if (content === '!dor') {

  const guildConfig = await Guild.findOne({ guildId: message.guild.id });

  if (!guildConfig || !guildConfig.hallChannelId) {
    return message.reply('❌ O canal do Hall da Fama não foi configurado ainda!');
  }

  const hallChannel = await client.channels.fetch(guildConfig.hallChannelId).catch(() => null);

  if (!hallChannel) {
    return message.reply('❌ Não consegui encontrar o canal do Hall da Fama.');
  }

  const frases = [
    '💀 tá impossível já...',
    '🪦 alguém chama um quiroprata',
    '😭 a coluna pediu arrego',
    '⚰️ ela não tankou mais',
    '🦴 coluna foi de base',
    '😵‍💫 ela virou um S',
  ];

  const frase = frases[Math.floor(Math.random() * frases.length)];

  let data = await Dor.findOne({ userId: AMORA_ID });

  if (!data) {
    data = new Dor({ userId: AMORA_ID, count: 0 });
  }

  data.count++;
  await data.save();

  const user = await client.users.fetch(AMORA_ID);

  return hallChannel.send(
    `💀 ${user} está com dor pela **${data.count}ª vez**\n${frase}`
  );
}
  //  COMANDOS ADMIN 
  if (message.member.permissions.has(PermissionFlagsBits.Administrator)) {
    if (content === '!apresentacao') {
      const introEmbed = new EmbedBuilder()
        .setTitle('🏆 BEM-VINDO AO HALL DA FAMA OFICIAL')
        .setColor(0xF1C40F)
        .setDescription(`*Aqui ficam registrados os maiores jogadores de cada temporada.*\n*Somente os 3 membros com mais Fama entram para a história.*\n\n📅 **Como funciona?**\n• Cada temporada tem duração definida.\n• Ao final, o Top 3 é registrado aqui.\n• A Fama é resetada para uma nova disputa.\n• Os vencedores recebem cargo exclusivo.\n\n🔥 **Quer entrar para o Hall da Fama?**\n*Participe, interaja e suba no ranking.*`);
      const msg = await message.channel.send({ embeds: [introEmbed] });
      await msg.pin().catch(() => {});
      return message.delete().catch(() => {});
    }

    if (content === '!bitesthedust') {
      await User.updateMany({}, { fama: 0 });
      let season = await Season.findOne() || new Season();
      season.seasonNumber = 1;
      season.start = new Date();
      await season.save();

      const embed = new EmbedBuilder()
        .setTitle('⏳ KILLER QUEEN: BITES THE DUST!')
        .setImage(BITES_THE_DUST_GIF)
        .setColor(0xFF0000);

      return message.channel.send({ embeds: [embed] });
    }

    if (content === '!sethall') {
      const channel = message.mentions.channels.first();
      if (!channel) return message.reply("❌ Mencione o canal.");
      await Guild.findOneAndUpdate({ guildId: message.guild.id }, { hallChannelId: channel.id }, { upsert: true });
      return message.reply(`✅ Canal configurado: ${channel}`);
    }

    if (content.startsWith('!addfama')) {
      const target = message.mentions.users.first();
      const amt = parseInt(args[2]);
      if (!target || isNaN(amt)) return message.reply("Use: !addfama @user valor.");
      let user = await User.findOne({ userId: target.id }) || new User({ userId: target.id });
      user.fama += amt;
      await user.save();
      return message.reply(`⭐ ${target} agora tem ${user.fama}`);
    }

    if (content.startsWith('!remfama')) {
      const target = message.mentions.users.first();
      const amt = parseInt(args[2]);
      if (!target || isNaN(amt)) return message.reply("Use: !remfama @user valor.");
      let user = await User.findOne({ userId: target.id }) || new User({ userId: target.id });
      user.fama -= amt;
      if (user.fama < 0) user.fama = 0;
      await user.save();
      return message.reply(`❌ ${target} agora tem ${user.fama} de fama`);
    }

    if (content === '!fimseason') {
      let season = await Season.findOne() || new Season({ seasonNumber: 1 });
      await sendSeasonEmbed(message.channel, season.seasonNumber);
      await User.updateMany({}, { fama: 0 });
      season.start = new Date();
      season.seasonNumber += 1;
      await season.save();
      return message.reply('✅ Temporada finalizada!');
    }

    if (content === '!resetseason') {
      let season = await Season.findOne();
      if (!season) {
        season = new Season({ seasonNumber: 1, start: new Date(), durationDays: 30 });
        await season.save();
      }
      await sendSeasonEmbed(message.channel, season.seasonNumber);
      await User.updateMany({}, { fama: 0 });
      season.start = new Date();
      await season.save();
      return message.reply(`🔄 Temporada resetada! Ainda estamos na Season ${season.seasonNumber}`);
    }
  }

//  GANHO DE FAMA AUTOMÁTICO 
if (
  !content.startsWith('!') &&
  isValidMessage(content, message.author.id) &&
  canGainPoint(message.author.id)
) {
  await User.findOneAndUpdate(
    { userId: message.author.id },
    { $inc: { fama: 1 } },
    { upsert: true }
  );
}

  // DUELO 
if (content.startsWith('!duelo')) {

  const guildConfig = await Guild.findOne({ guildId: message.guild.id });
  if (!guildConfig || message.channel.id !== guildConfig.hallChannelId) {
    return message.reply('❌ Esse comando só pode ser usado no canal do Hall da Fama!');
  }
    const target = message.mentions.users.first();
    const aposta = parseInt(args[2]);

    if (!target || isNaN(aposta) || aposta <= 0) return message.reply('❌ Use: !duelo @alvo quantidade (quantidade > 0)');
    if (target.id === message.author.id) return message.reply('❌ Você não pode duelar consigo mesmo!');
    
    const authorData = await User.findOne({ userId: message.author.id }) || new User({ userId: message.author.id });
    const targetData = await User.findOne({ userId: target.id }) || new User({ userId: target.id });

    if (authorData.fama < aposta) return message.reply('❌ Você não tem fama suficiente para apostar isso!');
    if (targetData.fama < aposta) return message.reply(`❌ ${target.username} não tem fama suficiente para essa aposta!`);

    const duelEmbed = new EmbedBuilder()
      .setTitle('🤠 DUELANDO NO HALL DA FAMA!')
      .setDescription(`${message.author} desafiou ${target} para um duelo apostando **${aposta}⭐**!`)
      .setImage(DUEL_GIF)
      .setColor(0xFFAA00)
      .setFooter({ text: 'Clique no botão para aceitar o duelo!' })
      .setTimestamp();

    const acceptButton = new ActionRowBuilder().addComponents(
      new ButtonBuilder()
        .setCustomId(`accept_duel_${message.author.id}_${target.id}_${aposta}`)
        .setLabel('ACEITAR DUELAR')
        .setStyle(ButtonStyle.Success)
    );

    const sentMessage = await message.channel.send({ embeds: [duelEmbed], components: [acceptButton] });

    const filter = (i) => i.customId === `accept_duel_${message.author.id}_${target.id}_${aposta}` && i.user.id === target.id;
    const collector = sentMessage.createMessageComponentCollector({ filter, time: 60000, max: 1 });

    collector.on('collect', async (i) => {
      await i.deferUpdate();
      const resultados = ['vitória','empate','vitória_bonus','derrota_reduzida'];
      const resultado = resultados[Math.floor(Math.random() * resultados.length)];
      let mensagemResultado = '';

      switch (resultado) {
        case 'vitória':
          authorData.fama += aposta;
          targetData.fama -= aposta;
          mensagemResultado = `${message.author} venceu o duelo! ⭐ ${aposta} pontos ganhos.`;
          break;
        case 'empate':
          mensagemResultado = `O duelo terminou empatado! Ninguém ganha pontos.`;
          break;
        case 'vitória_bonus':
          const bonus = Math.ceil(aposta * 1.5);
          authorData.fama += bonus;
          targetData.fama -= aposta;
          mensagemResultado = `✨ Vitória épica de ${message.author}! Ganhou **${bonus}⭐**!`;
          break;
        case 'derrota_reduzida':
          const perda = Math.ceil(aposta / 2);
          authorData.fama -= perda;
          targetData.fama += perda;
          mensagemResultado = `😅 ${message.author} perdeu, mas a perda foi reduzida: -${perda}⭐`;
          break;
      }

      if (authorData.fama < 0) authorData.fama = 0;
      if (targetData.fama < 0) targetData.fama = 0;
      await authorData.save();
      await targetData.save();

      const resultEmbed = new EmbedBuilder()
        .setTitle('⚡ RESULTADO DO DUELO')
        .setDescription(mensagemResultado)
        .setColor(0x00FF00)
        .setTimestamp();

      await message.channel.send({ embeds: [resultEmbed] });
      sentMessage.edit({ components: [] });
    });

    collector.on('end', async (collected) => {
      if (!collected.size) {
        await sentMessage.edit({ components: [] });
        await message.channel.send('⏰ O duelo expirou! Ninguém aceitou a aposta.');
      }
    });
  }

  //  RANKING 
  if (content === '!ranking') {
    const top = await User.find({ fama: { $gt: 0 } }).sort({ fama: -1 }).limit(10);
    const embed = new EmbedBuilder()
      .setTitle('🏆 RANKING DE FAMA - TOP 10')
      .setColor(0xF1C40F)
      .setThumbnail(RANKING_BANNER)
      .setDescription(
        top.map((u, i) => `${i<3?['🥇','🥈','🥉'][i]:`**${i+1}º**`} <@${u.userId}> — ⭐ **${u.fama}**`).join('\n') || "Vazio."
      );
    return message.reply({ embeds: [embed] });
  }

  //  FAMA 
  if (content === '!fama') {
    const user = await User.findOne({ userId: message.author.id });
    return message.reply(`⭐ Sua fama atual: **${user ? user.fama : 0}**`);
  }

  //  DONO 
  if (content === '!restart' && message.author.id === SEU_ID_DONO) {
    await message.reply("🔄 Reiniciando...");
    return process.exit();
  }
});

//  LOGIN 
client.login(process.env.TOKEN);